...
import { TrendingUp, ShoppingBag, CheckCircle, DollarSign, BarChart2, Award } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24">
      <div className="mt-2 mb-6">
        ...
        <p className="text-noir-400 text-sm">Vue d'ensemble de l'activité</p>
      </div>
        ...
        <div className="card p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-brand-500 text-xl">{stats.todayRevenue.toLocaleString('fr-FR')} FCFA</span>
            </div>
          </div>
        </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1.5">
                ...
                <span className="font-semibold text-crimson-700">
                  {stats.totalOrders > 0 ? Math.round((stats.refusedOrders / stats.totalOrders) * 100) : 0}%
                </span>
              </div>
              <div className="h-2 bg-noir-100 rounded-full overflow-hidden">
                <div
                  className="h-full bg-crimson-600 rounded-full transition-all duration-1000"
                  style={{ width: `${stats.totalOrders > 0 ? (stats.refusedOrders / stats.totalOrders) * 100 : 0}%` }}
                />
              </div>
            </div>
          </div>
            ...
            {stats.recentOrders.map(order => (
              <div key={order.id} className="flex items-center justify-between py-2 border-b border-noir-50 last:border-0">
                ...
                <div className="text-right">
                  <div className="font-bold text-brand-500 text-sm">{order.total.toLocaleString('fr-FR')} FCFA</div>
